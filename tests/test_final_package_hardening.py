        ...
        writer.writerow(
            {
                ...
                "active_nibbles": "1",
                "generation_command": (
                    "saved_certified_source_csv;"
                    " historical_generator=repository_only_candidate_miner_approx;"
                    " final_package_generator=no;"
                    " parameters=beam/trans/branch"
                ),
                "beam": "",
                "trans": "",
                "branch": "",
...
def test_package_checker_fixture() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        manifest_path.write_text(
            text.replace(
                hashlib.sha256((source_root / "experiments" / "r2_fixture.csv").read_bytes()).hexdigest(),
                "0" * 64,
                1,
            ),
            encoding="utf-8",
...
def test_committed_package_boundary() -> None:
    ...
    for path in [
        ...
        ROOT / "docs" / "OFFICIAL_SPEC_INTERPRETATION.md",
        ROOT / "docs" / "EVIDENCE_SCOPE.md",
        PACKAGE_ROOT / "README_FIRST.md",
        PACKAGE_ROOT / "docs" / "EVIDENCE_SCOPE.md",
        PACKAGE_ROOT / "docs" / "SUBMISSION_MANIFEST.md",
        PACKAGE_ROOT / "docs" / "REPRODUCIBILITY.md",
    ]:
        text = path.read_text(encoding="utf-8")
        if path.name == "EVIDENCE_SCOPE.md":
            require("vt_provenance_closed=false" in text, f"boundary flags missing in {path}")
            continue
        if path.name == "README_FIRST.md":
            require("docs/EVIDENCE_SCOPE.md" in text, "README_FIRST must point to package evidence scope")
            require("full_2_32_run_started=false" not in text, "README_FIRST should not repeat the full boundary flag block")
            require("check_submission_package.py" in text, "README_FIRST must retain the package-safe checker command")
            continue
        if path.name == "REPRODUCIBILITY.md" and "submission_final" in path.as_posix():
            require("EVIDENCE_SCOPE.md" in text, f"package reproducibility must point to evidence scope: {path}")
            require("check_submission_package.py" in text, f"package-safe checker command missing in {path}")
            continue
        require("check_submission_package.py" in text or "EVIDENCE_SCOPE.md" in text, f"expected package-safe boundary note missing in {path}")
    boundary_text = (PACKAGE_ROOT / "docs" / "SUBMISSION_MANIFEST.md").read_text(encoding="utf-8")
    require("历史候选发现标签" in boundary_text or "historical discovery label" in boundary_text, "SOURCE_MANIFEST boundary note missing from package submission manifest")


def test_source_manifest_semantics_are_package_safe() -> None:
    manifest_path = ROOT / "experiments" / "SOURCE_MANIFEST.csv"
    with manifest_path.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    require(rows, "SOURCE_MANIFEST.csv is empty")
    used_rows = [row for row in rows if row.get("used_in_final_submit") == "1"]
    require(used_rows, "SOURCE_MANIFEST.csv lists no final-submit rows")
    for row in used_rows:
        command = row["generation_command"]
        require("saved_certified_source_csv" in command, f"SOURCE_MANIFEST final-package semantics missing for {row['path']}")
        require("final_package_generator=no" in command, f"SOURCE_MANIFEST must reject rerun semantics for {row['path']}")
        require("historical_generator=repository_only_candidate_miner_approx" in command or "historical_generator=legacy_search_candidates" in command, f"historical generator note missing for {row['path']}")
        require("docs/EXPERIMENTS.md" not in command, f"SOURCE_MANIFEST still points to docs/EXPERIMENTS.md for {row['path']}")
...
def test_way2_boundary_language_and_lat_basis_present() -> None:
    for path in [ROOT / "参赛论文" / "参赛论文_赛题三_稳稳接住.tex", ROOT / "REPORT.md"]:
        text = path.read_text(encoding="utf-8")
        require("方式二审计复现不匹配数为 0" in text or "方式二数值复现不匹配数为 0" in text, f"way-2 mismatch wording missing in {path}")
        require(
            "局部 S 盒相关度来自归一化 Walsh 和" in text
            and ("{-8,-4,4,8,16}" in text or "\\{-8,-4,4,8,16\\}" in text),
            f"LAT / denominator basis note missing in {path}",
        )
...
def test_root_sha_manifest_covers_tracked_files() -> None:
    ...
    generated_paths = {
        ...
        for path in (ROOT / "submission_final").rglob("*")
        if path.is_file()
    }
    if (ROOT / "submission_final.zip").is_file():
        generated_paths.add("submission_final.zip")
    expected = tracked_paths | generated_paths
    entries = root_sha_entries()
    require(
        entries == expected,
        "root SHA256SUMS.txt does not exactly cover tracked files plus generated package artifacts",
    )
    require(
        "experiments/check_submission_package.py" in entries,
        "root SHA256SUMS.txt is missing the package-safe checker source",
    )
    require(
        "submission_final/source/experiments/check_submission_package.py" in entries,
        "root SHA256SUMS.txt is missing the packaged checker",
    )
...
def main() -> int:
    ...
    test_source_manifest_semantics_are_package_safe()
    test_paper_field_mapping_moved_to_appendix()
    test_way2_boundary_language_and_lat_basis_present()
