...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check only the runnable final-package submission boundary.")
...
def load_source_manifest(path: Path) -> list[dict[str, str]]:
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        require(reader.fieldnames == MANIFEST_FIELDS, f"unexpected SOURCE_MANIFEST schema: {reader.fieldnames}")
...
def verify_source_manifest(source_root: Path) -> None:
    ...
    for row in rows:
        ...
        require(path.resolve().is_relative_to(source_root.resolve()), f"SOURCE_MANIFEST path escapes source root: {rel}")
...
def verify_submit_rebuild(source_root: Path, submit_path: Path, expected_submit_sha256: str) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        require(rebuilt.read_bytes() == submit_path.read_bytes(), "build_submit_from_sources.py did not reproduce submit.txt")
    require(sha256_file(submit_path) == expected_submit_sha256, "submit.txt SHA no longer matches the frozen final package")


def verify_score_output(source_root: Path, submit_path: Path, expected_valid_count: int, expected_total_score: str) -> None:
    score_bin = source_root / "score"
    require(score_bin.is_file(), "score executable is missing; build the package source first")
    output = run([str(score_bin), "--dedup", "uv", "--positive-only", str(submit_path)], cwd=source_root)
...
def main() -> int:
    ...
    submit_path = (source_root / args.submit).resolve() if not Path(args.submit).is_absolute() else Path(args.submit)
    ...
    verify_score_output(source_root, submit_path, args.expected_valid_count, args.expected_total_score)
