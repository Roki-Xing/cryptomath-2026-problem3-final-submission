...
{\songti\zihao{4}
本文研究由分组密码轮函数相关矩阵诱导的结构化坐标计算问题. 给定轮数 $r$、输入掩码 $u$ 与输出掩码 $v$, 目标是计算由 $3r$ 个 $2^{32}\times 2^{32}$ 相关矩阵连乘得到的坐标 $\bm{M}(r)[v,u]$. 直接精确路径需枚举 $2^{32}$ 个明文, 难以支撑大规模候选发现. 本文提出可认证稀疏路线壳求和: 利用八个并联 S 盒相关矩阵的克罗内克积结构枚举非零局部分支, 利用行移位与列混合层的转置传播关系确定唯一后继掩码, 并对到达同一掩码的路线贡献进行带符号聚合. 候选阶段允许截断以发现高价值端点; 最终结果仅保留局部分支、S 层元组和聚合状态均未截断的运行. 本文证明, 对固定 $(r,u)$, 该完整性证书蕴含 $A_r=\bm{R}^{r}e_u$, 从而任意输出端点满足 $A_r[v]=\bm{M}(r)[v,u]$. 在评测实例上, 按当前冻结结果文件和本文自实现评分程序口径, 最终结果集包含 138338 条正分记录, 总分为 105843.622442471292742994; 全部记录通过无截断审计, 方式二审计复现不匹配数为 0. 18 条方式一独立精确抽样复核全部一致, 其中包括 4 条 $r=3$ 活跃二字补充样本. 结果表明, 本文方法能够在不枚举明文空间的前提下, 对低活跃掩码区域中的完整线性壳进行可证明、可审计的稀疏求和. }
...
\subsection{数值表示与精确复算}

定理 1 在实数域中的相关矩阵上成立. 局部 S 盒相关度来自归一化 Walsh 和, 因此每条路线贡献和完整路线壳和均为二进有理数. 对本评测实例的固定 S 盒, 穷尽 $16\times16$ 个局部 Walsh 分子可得非零集合为 $\{-8,-4,4,8,16\}$, 因此局部相关度属于 $\{\pm 1/2,\pm 1/4,1\}$. 八个并联 S 盒的一轮贡献可统一置于分母 $2^{16}$ 下. 对本评测实例, 精确方式二复算使用整数分子与二进制指数表示状态值. 对固定 $(r,u)$, 完整路线壳值可表示为
\begin{equation}
A_r[v]=\frac{N_r[v]}{2^{16r}},
\label{eq:dyadic-exact-value}
\end{equation}
其中 $N_r[v]$ 为有符号整数. 复现材料中的精确方式二复算覆盖全部 4760 个唯一 $(r,u)$ 列, 并对 138338 个冻结端点与提交字段中的方式二数值进行精确比较; 比较结果全部一致. 该证据闭合方式二数值来源, 但不等同于全量方式一 $VT$ 来源链已经闭合. 零值过滤和去重在输出构造阶段执行, 不参与定理 1 的证明.
...
\subsection{冻结结果文件的记录数与自算得分}

按当前冻结结果文件和本文自实现评分程序口径, 最终结果集包含 138338 条正分记录. 按评测规则计算, 总分为 105843.622442471292742994. 按轮数统计见表~\ref{tab:score}.

\begin{table}[H]
\centering
\caption{冻结结果文件的记录数与自算得分}
\label{tab:score}
\zihao{-4}
\begin{tabular}{rrrr}
\toprule
轮数 $r$ & 正分记录数 & 分数和 & 最高单条分数 \\
\midrule
1 & 288 & 288.000000 & 1 \\
...
方式一精确程序仅在最终结果生成完成后读取抽样端点, 独立计算参考值并进行一致性比较; 它不参与候选生成、排序、筛选或数值写入. 表~\ref{tab:spotcheck} 给出代表性样本, 完整摘要为 18 条查询、4 条线性近似表引导批次活跃二字样本、不匹配数 0、最大绝对误差 0、最大相对误差 0.
...
活跃二字正值样本 & 3 & \code{0x00006060} & \code{0x04004004} & 0.125 & 0.125 & 是 \\
活跃二字负值样本 & 3 & \code{0x00006060} & \code{0x08008008} & -0.125 & -0.125 & 是 \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{精确复算与独立工具链验证}

最终收口阶段追加了全部冻结端点的方式二精确复算. 该复算覆盖冻结结果中的 4760 个唯一 $(r,u)$ 输入列, 并由两个独立整数后端分别完成全部列. 所有列均通过精确认证和 Parseval 检查; 138338 个冻结端点与提交字段中的方式二数值全部一致; 未发现数值解析错误、缺失端点或两个后端之间的不一致. 该证据闭合的是方式二数学与数值证据链, 对应复现材料位于 \code{artifacts/way2\_exact/full/}.
...
S 盒层的局部线性近似表决定从一个活跃四比特字出发的非零分支数量和局部相关强度. 对于三轮低活跃掩码搜索, 输入活跃位置与局部掩码取值会显著影响路线空间展开规模. 因此, 线性近似表引导的三轮活跃二字补充搜索(复现材料中记为 E06)优先考虑活跃四比特字数为 2 且局部 LAT 分支结构较有利的输入模式.

\subsection{合入规则}

从该补充批次的候选来源中选取满足以下条件的记录: $r=3$; 活跃四比特字数为 2; 通过无截断认证; 单条分数为正; 输入掩码、输出掩码和数值均非零; 且不与既有 $(u,v)$ 冲突. 筛选仅使用方式二认证来源和审计信息, 不使用方式一精确输出进行候选排序、筛选或数值生成.

\subsection{结果}

该补充搜索最终合入 384 条记录, 涉及 6 个唯一输入掩码. 所有合入记录均包含在最终全量审计中, 并满足与其余记录相同的无截断认证条件. 4 条该补充批次样本进入方式一精确抽样复核, 不匹配数为 0.
...
最终结果文件包含评测格式要求的两个数字字段 $VT$ 与 $VE$. 对无截断认证记录, 方式二完整路线壳求和得到的数值等于目标相关矩阵坐标. 因此, 当前冻结结果文件中的两个数字字段具有相同数值. 这里的相同数值是方式二精确复算得到的数值事实; 它不表示这些历史 $VT$ 字段已经由完整方式一明文枚举链生成. 该数值来自方式二的可认证完整路线壳求和, 不是复制方式一明文枚举输出. 方式一全量 $VT$ provenance 尚未闭合; 当前方式一证据包括 18 条独立抽样复核和方式一批处理工具链小域一致性验证.
...
在评测实例上, 按当前冻结结果文件和本文自实现评分程序口径, 最终结果集包含 138338 条正分记录, 总分为 \seqsplit{105843.622442471292742994}. 全部记录均通过无截断认证, 方式二审计复现不匹配数为 0, 不存在零掩码、零值和重复端点; 进一步的精确方式二复算覆盖全部 4760 个唯一输入列, 并确认 138338 个冻结端点与提交字段中的方式二数值一致. 18 条方式一独立抽样复核与方式二认证值完全一致; 方式一批处理工具链的小域验证也未发现不一致. 本文结果由数学模型、正确性定理、按唯一 $(r,u)$ 的复杂度分析、全量无截断审计、精确方式二复算和独立抽样复核共同支撑, 同时不声称全量方式一 $VT$ 来源链已经闭合.
...
评测实例给定一个 32 比特替换-置换网络结构, 一轮为 $F=MC\circ SR\circ SC$. 相关矩阵连乘定义为 $\bm{M}(r)=\bm{M}_{3r-1}\bm{M}_{3r-2}\cdots\bm{M}_0$. 每条评测记录包含 $(r,u,v,VT,VE)$. 对本文最终无截断认证记录, 方式二完整路线壳求和给出 $\bm{M}(r)[v,u]$, 因此两个数字字段具有相同数值. 这里的相同数值是方式二精确复算得到的数值事实; 它不表示这些历史 $VT$ 字段已经由完整方式一明文枚举链生成. 单条有效记录得分为
\[
...
复现流程包括编译与测试、结果文件重建、评分、全量审计、复杂度摘要、full exact-way2 证据检查、Strategy-B Stage-A 证据检查和方式一抽样复核. 下列为仓库复现材料中的状态名, 用于说明证据边界, 不作为正文数学结论的名称. 主要命令如下:

复现材料中的逐行审计记录以顶层字段 \texttt{certified\_no\_truncation} 表示定义 1 的三项结构条件是否同时满足. 每轮 \texttt{round\_stats} 记录 \texttt{branch\_truncated\_states}、\texttt{tuple\_truncated\_states} 与 \texttt{beam\_pruned}. 在本文最终结果对应的审计记录中, 三项条件分别表现为局部分支截断数为 0、S 层元组截断数为 0、未发生状态剪枝. 上述字段是定义 1 的实现审计证据, 不作为新的数学假设.
...
\caption{方式二结果生成链与方式一独立抽样复核链的边界. 最终结果由方式二可认证稀疏路线壳求和产生; 方式一仅在结果生成完成后读取抽样端点并独立计算参考值, 比较结果只用于复核, 不参与候选生成、排序、筛选或提交文件构造. }
