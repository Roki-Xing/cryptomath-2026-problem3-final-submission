...
exact_oracle / exact_batch_mt
  ...
  -> used only for validation-only spotchecks
  -> never used to generate, rank, filter, or populate submitted VT/VE values

experiments/build_submit_from_sources.py / score
  -> load certified way-2 source rows
  -> write the certified route-shell value into the two required submission fields
  -> reject duplicate or non-positive-score rows
  -> emit the final submit.txt without reading exact_oracle output
```

For certified no-truncation sparse-DP rows, the way-2 route-shell dynamic program enumerates and sums all nonzero linear routes from `u` to `v`. By the correlation-matrix product expansion, this complete route-shell sum is `M(r)[v,u]`. Therefore the two numeric fields required by the contest format, `VT` and `VE`, carry the same value on these certified rows. This value is produced by the way-2 structured computation; it is not copied from way-1 plaintext enumeration output.
...
- `docs/VT_VE_COMPLIANCE.md`: compliance explanation for the two numeric fields in `submit.txt`.
